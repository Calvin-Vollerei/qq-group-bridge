...
**最新版本**的 `qgb-<版本>-full.zip`（约 52 MB），解压到任意目录，双击 `启动搬运工.bat` 即可。
...
> 只想下载主程序、NapCat 另外装？用同版本的 `qgb-<版本>-app-only.zip`。
...
python scripts/pack_release.py --version v1.5

rem 合成给零基础用户的交付包（中文包名、求救脚本改名、附带三份图文说明）
python scripts/make_delivery_zip.py --version v1.5
```
...
1. `powershell -File scripts/build.ps1` —— 跑全部闸门并生成 `dist/QQ群文件搬运工/`
   与 `dist/qgb-v1.5-app-only.zip`（**不含** `data/`，即不含任何凭据）
2. 在该目录里完成一次真实登录（NapCat 组件才会就位），再
   `python scripts/pack_release.py --version v1.5` 合成整包
3. 把 `dist/qgb-v1.5-full.zip` 拖进 GitHub 的 Release
